...
function MPKEditor()
{
    ...
    function readData(event)
    {
        ...
        else if($MPK !== undefined && isNoteFile(data) === true)
        {
            importNote(data, $MPK);
        }
    }
    ...
    function checkKeyIndexes(noteTableKeys, indexTableKeys) {
        var i;
        if (noteTableKeys.length !== indexTableKeys.length) {
            return false;
        }
        for (i = 0; i < noteTableKeys.length; i++) {
    ...
    function init()
    {
        function A(a) {for(i=0;i<7;++i) {d[a+i]=[1,1,0,1,1,254,241][i];}}
        var i, d = new Uint8Array(32768), newMPK, initMemPak;
...
window.MPKEditor = new MPKEditor();
}());
